...
Det finnes mange forskjellige temaer i R. Man lærer seg fort hvilke tema man liker, men jeg har valgt ut et par som kan falle i god smak for de fleste. Siden ggplot-funksjonen vår begynner å bli litt lang og vi ønsker å leke litt med temaer, kan det være en idé å legge selve plottet i et eget element. Da kan vi enkelt gjøre justeringer på selve temaet uten å gjengi all kode hver gang.

Vi legger derfor ggplot-funksjonen vår i et element kalt `g`. Deretter skriver vi navnet på elementet **pluss** de ulike temaene vi ønsker å teste ut.
...
Noen tema jeg liker veldig godt er `theme_light()` og `theme_bw()`. `theme_void()`er ofte brukt for visualiseringer som ikke nødvendigvis er knyttet opp mot et todimensjonalt plott med x- og y-akse.
